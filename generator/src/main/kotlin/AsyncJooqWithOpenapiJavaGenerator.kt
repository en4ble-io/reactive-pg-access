...
//                || comment.contains("{{readOnly}}")
    ...
    override fun generateDaoClassFooter(table: TableDefinition, pojoType: PojoType, out: JavaWriter) {
        ...
        out.tab(1).println("override fun rxReadCount(clientId: String?):io.reactivex.Single<Int> {")
        out.tab(1).println("return rxReadCount(dsl.selectCount().from($fullJavaTableName), clientId)")
        out.tab(1).println("}")
        ...
        out.tab(1)
            .println("override fun rxRead(condition:org.jooq.Condition, clientId: String?):io.reactivex.Single<List<$dtoType>> {")
        out.tab(1).println("return rxRead(condition, $fullJavaTableName, clientId)")
        out.tab(1).println("}")
        out.tab(1)
            .println("override fun rxRead(condition:org.jooq.Condition,client:io.vertx.reactivex.sqlclient.SqlClient, clientId: String?):io.reactivex.Single<List<$dtoType>> {")
        out.tab(1).println("return rxRead(condition, $fullJavaTableName, client, clientId)")
        out.tab(1).println("}")
        ...
        out.tab(1)
            .println("override fun rxRead(query:org.jooq.Query, clientId: String?):io.reactivex.Single<List<$dtoType>> {")
        out.tab(1).println("return rxRead(query, $fullJavaTableName, clientId)")
        out.tab(1).println("}")
        out.tab(1)
            .println("override fun rxRead(query:org.jooq.Query,client:io.vertx.reactivex.sqlclient.SqlClient, clientId: String?):io.reactivex.Single<List<$dtoType>> {")
        out.tab(1).println("return rxRead(query, $fullJavaTableName, client, clientId)")
        out.tab(1).println("}")
        ...
        out.tab(1)
            .println("override fun rxReadOne(condition:org.jooq.Condition, clientId: String?):io.reactivex.Single<$dtoType> {")
        out.tab(1).println("return rxReadOne(condition, $fullJavaTableName, clientId)")
        out.tab(1).println("}")
        out.tab(1)
            .println("override fun rxReadOne(condition:org.jooq.Condition,client:io.vertx.reactivex.sqlclient.SqlClient, clientId: String?):io.reactivex.Single<$dtoType> {")
        out.tab(1).println("return rxReadOne(condition, $fullJavaTableName, client, clientId)")
        out.tab(1).println("}")
        ...
        out.tab(1)
            .println("override fun rxReadOne(query:org.jooq.Query, clientId: String?):io.reactivex.Single<$dtoType> {")
        out.tab(1).println("return rxReadOne(query, $fullJavaTableName, clientId)")
        out.tab(1).println("}")
        out.tab(1)
            .println("override fun rxReadOne(query:org.jooq.Query,client:io.vertx.reactivex.sqlclient.SqlClient, clientId: String?):io.reactivex.Single<$dtoType> {")
        out.tab(1).println("return rxReadOne(query, $fullJavaTableName, client, clientId)")
        out.tab(1).println("}")
        ...
        out.tab(1)
            .println("override fun rxReadOptional(condition:org.jooq.Condition, clientId: String?):io.reactivex.Single<java.util.Optional<$dtoType>> {")
        out.tab(1).println("return rxReadOptional(condition, $fullJavaTableName, clientId)")
        out.tab(1).println("}")
        out.tab(1)
            .println("override fun rxReadOptional(condition:org.jooq.Condition,client:io.vertx.reactivex.sqlclient.SqlClient, clientId: String?):io.reactivex.Single<java.util.Optional<$dtoType>> {")
        out.tab(1).println("return rxReadOptional(condition, $fullJavaTableName, client, clientId)")
        out.tab(1).println("}")
        ...
        out.tab(1)
            .println("override fun rxReadOptional(query:org.jooq.Query, clientId: String?):io.reactivex.Single<java.util.Optional<$dtoType>> {")
        out.tab(1).println("return rxReadOptional(query, $fullJavaTableName, clientId)")
        out.tab(1).println("}")
        out.tab(1)
            .println("override fun rxReadOptional(query:org.jooq.Query,client:io.vertx.reactivex.sqlclient.SqlClient, clientId: String?):io.reactivex.Single<java.util.Optional<$dtoType>> {")
        out.tab(1).println("return rxReadOptional(query, $fullJavaTableName, client, clientId)")
        out.tab(1).println("}")
        ...
        if (!readonly) {
            ...
            out.tab(1)
                .println("fun rxUpdate(dto:$dtoType, condition:org.jooq.Condition, clientId: String?):io.reactivex.Single<Int> {")
            printMapper(out, mapperGetter, reactive = true, update = true)
            out.tab(2)
                .println("return rxQuery(dsl.update($fullJavaTableName).set(map).where(condition), clientId).map{ it.delegate.rowCount() }")
            out.tab(1).println("}")
            ...
            out.tab(1)
                .println("fun rxUpdate(dto:$dtoType, condition:org.jooq.Condition, client:io.vertx.reactivex.sqlclient.SqlClient, clientId: String?):io.reactivex.Single<Int> {")
            printMapper(out, mapperGetter, reactive = true, update = true)
            out.tab(2)
                .println("return rxQuery(dsl.update($fullJavaTableName).set(map).where(condition),client, clientId).map{ it.delegate.rowCount() }")
            out.tab(1).println("}")
            ...
            out.tab(1)
                .println("fun rxUpdateReturning(dto:$dtoType, condition:org.jooq.Condition, clientId: String?):io.reactivex.Single<List<$dtoType>> {")
            printMapper(out, mapperGetter, reactive = true, update = true, returningList = true)
            out.tab(2)
                .println("return rxQuery(dsl.update($fullJavaTableName).set(map).where(condition).returning(*$fullJavaTableName.fields()), clientId).map{ map(it.delegate as io.vertx.sqlclient.RowSet<io.vertx.sqlclient.Row>) }")
            out.tab(1).println("}")

            out.tab(1)
                .println("fun rxUpdateReturningOne(dto:$dtoType, condition:org.jooq.Condition, clientId: String?):io.reactivex.Single<$dtoType> {")
            printMapper(out, mapperGetter, reactive = true, update = true, returningOne = true)
            out.tab(2)
                .println("return rxQueryOne(dsl.update($fullJavaTableName).set(map).where(condition).returning(*$fullJavaTableName.fields()), clientId).map{ map(it.delegate) }")
            out.tab(1).println("}")
            ...
            out.tab(1)
                .println("fun rxUpdateReturning(dto:$dtoType, condition:org.jooq.Condition, client:io.vertx.reactivex.sqlclient.SqlClient, clientId: String?):io.reactivex.Single<$dtoType> {")
            printMapper(out, mapperGetter, reactive = true, update = true, returningOne = true)
            out.tab(2)
                .println("return rxQueryOne(dsl.update($fullJavaTableName).set(map).where(condition).returning(*$fullJavaTableName.fields()),client, clientId).map{ map(it.delegate) }")
            out.tab(1).println("}")
            ...
            out.tab(1).println("fun rxCreate(dto:$dtoType, clientId: String?):io.reactivex.Single<Int> {")
            printMapper(out, mapperGetter, reactive = true, validate = true, update = false)
            out.tab(2)
                .println("return rxQuery(dsl.insertInto($fullJavaTableName).columns(map.keys).values(map.values), clientId).map{ it.delegate.rowCount() }")
            out.tab(1).println("}")

            out.tab(1)
                .println("fun rxCreate(dto:$dtoType,client:io.vertx.reactivex.sqlclient.SqlClient, clientId: String?):io.reactivex.Single<Int> {")
            out.tab(2)
            printMapper(out, mapperGetter, reactive = true, validate = true, update = false)
            out.tab(2)
                .println("return rxQuery(dsl.insertInto($fullJavaTableName).columns(map.keys).values(map.values), client, clientId).map{ it.delegate.rowCount() }")
            out.tab(1).println("}")

            out.tab(1).println("fun rxCreateReturning(dto:$dtoType, clientId: String?):io.reactivex.Single<$dtoType> {")
            printMapper(out, mapperGetter, reactive = true, validate = true, update = false, returningList = true)
            out.tab(2)
                .println("return rxQueryOne(dsl.insertInto($fullJavaTableName).columns(map.keys).values(map.values).returning(*$fullJavaTableName.fields()), clientId).map{ map(it.delegate) }")
            out.tab(1).println("}")

            out.tab(1)
                .println("fun rxCreateReturning(dto:$dtoType,client:io.vertx.reactivex.sqlclient.SqlClient, clientId: String?):io.reactivex.Single<$dtoType> {")
            printMapper(out, mapperGetter, reactive = true, validate = true, update = false, returningOne = true)
            out.tab(2)
                .println("return rxQueryOne(dsl.insertInto($fullJavaTableName).columns(map.keys).values(map.values).returning(*$fullJavaTableName.fields()), client, clientId).map{ map(it.delegate) }")
            out.tab(1).println("}")
        }
    }
...
//        }
        ...
        for (column in table.columns) {
            ...
            if ("java.lang.Object" != colTypeFull) { // don't generate readBy methods for unknown types
                ...
                if (!colType.endsWith("[]")) {
                    ...
                    out.tab(1).println("}")
                    out.tab(1)
                        .println("fun rxReadBy$colClass(vararg values:$colType, clientId: String?):io.reactivex.Single<List<$pType>> {")
                    out.tab(2)
                        .println("return rxRead($colIdentifier.`in`(values.toList()),$fullJavaTableName, clientId)")
                    out.tab(1).println("}")

                    out.tab(1)
                        .println("suspend fun readBy$colClass(vararg values:$colType, orderBy: List<io.en4ble.pgaccess.dto.OrderDTO>, clientId: String?):List<$pType> {")
                    out.tab(2)
                        .println("return read($colIdentifier.`in`(values.toList()),$fullJavaTableName, orderBy)")
                    out.tab(1).println("}")
                    out.tab(1)
                        .println("fun rxReadBy$colClass(vararg values:$colType, orderBy: List<io.en4ble.pgaccess.dto.OrderDTO>, clientId: String?):io.reactivex.Single<List<$pType>> {")
                    out.tab(2)
                        .println("return rxRead($colIdentifier.`in`(values.toList()),$fullJavaTableName, orderBy, clientId)")
                    out.tab(1).println("}")
                    ...
                    out.tab(1).println("}")
                    out.tab(1)
                        .println("fun rxReadBy$colClass(vararg values:$colType, page: io.en4ble.pgaccess.dto.PagingDTO, clientId: String?):io.reactivex.Single<List<$pType>> {")
                    out.tab(2)
                        .println("return rxRead($colIdentifier.`in`(values.toList()),$fullJavaTableName, page, clientId)")
                    out.tab(1).println("}")
                }
                // readOneBy[Column]([T])
                // -----------------------
    ...
    private fun generateReadOneByMethods(
        ...
        out.tab(1).println("}")
        out.tab(1).println("fun rxReadOneBy$colClass(value:$colType, clientId: String?):io.reactivex.Single<$pType> {")
        out.tab(2).println("return rxReadOne($colIdentifier.eq(value),$fullJavaTableName, clientId)")
        out.tab(1).println("}")
        ...
        out.tab(1).println("}")
        out.tab(1)
            .println("fun rxReadOptionalBy$colClass(value:$colType, clientId: String?):io.reactivex.Single<java.util.Optional<$pType>> {")
        out.tab(2).println("return rxReadOptional($colIdentifier.eq(value),$fullJavaTableName, clientId)")
        out.tab(1).println("}")
    }
